...
class HistogramaPlugin(PluginBase):
    ...
    def setup_ui(self):
        ...
        self._modo_gray = QCheckBox("Exibir em escala de cinza")
        self._modo_gray.setChecked(False)
        self._modo_gray.stateChanged.connect(self._gerar_histograma)
        ...
        layout.addWidget(self._modo_gray)
    ...
    def _gerar_histograma(self):
        ...
        if self._modo_gray.isChecked() or self._canal_ativo == "Cinza":
    ...
    def _desenhar_histograma_slow(self, hists, cores):
        altura = 300
        largura = 512
        hist_img = np.zeros((altura, largura, 3), dtype=np.uint8)
    ...
    def _desenhar_histograma_using_lines(self, hists, cores):
        altura = 300
        largura = 512
        hist_img = np.zeros((altura, largura, 3), dtype=np.uint8)

        x = np.arange(256) * 2  # [0,2,4,...]
        ...
        for hist, cor in zip(hists, cores):
            ...
            cv2.polylines(hist_img, [pts], isClosed=False, color=cor, thickness=2)
