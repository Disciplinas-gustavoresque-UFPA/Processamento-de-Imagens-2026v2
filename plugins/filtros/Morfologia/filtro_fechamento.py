...
class FiltroFechamento(PluginBase):
    """Plugin que aplica o filtro morfológico de Fechamento em imagens RGB.

    O fechamento consiste em uma dilatação seguida de erosão. Preenche
    pequenos buracos e lacunas escuras dentro dos objetos sem alterar
    significativamente o tamanho e a forma dos objetos maiores. O efeito
    é controlado pela forma e tamanho do elemento estruturante e pelo
    número de iterações.
    """

    display_name = "Fechamento"
    ...
    def setup_ui(self) -> None:
        ...
        # --- Informativo ---
        self.info = QLabel(
            "Aplica dilatação seguida de erosão, preenchendo\n"
            "pequenos buracos e lacunas escuras sem alterar\n"
            "significativamente o tamanho dos objetos.",
            self,
        )
    ...
    def processar(self, imagem: np.ndarray) -> np.ndarray:
        """Aplica o fechamento morfológico na imagem.

        O fechamento é definido como uma dilatação seguida de erosão
        (``cv2.MORPH_CLOSE``).
        ...
        -------
        np.ndarray
            Imagem após a operação de fechamento.
        """
        tamanho = self._obter_tamanho_kernel()
        ...
        if possui_alpha:
            rgb = imagem[..., :3]
            alpha = imagem[..., 3]
            resultado_rgb = cv2.morphologyEx(
                rgb, cv2.MORPH_CLOSE, kernel, iterations=iteracoes,
            )
            resultado = np.dstack((resultado_rgb, alpha))
        else:
            resultado = cv2.morphologyEx(
                imagem, cv2.MORPH_CLOSE, kernel, iterations=iteracoes,
            )
