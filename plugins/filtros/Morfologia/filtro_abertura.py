...
class FiltroAbertura(PluginBase):
    """Plugin que aplica o filtro morfológico de Abertura em imagens RGB.

    A abertura consiste em uma erosão seguida de dilatação. Remove
    pequenas saliências e ruídos claros sem alterar significativamente
    o tamanho e a forma dos objetos maiores. O efeito é controlado pela
    forma e tamanho do elemento estruturante e pelo número de iterações.
    """

    display_name = "Abertura"
    ...
    def setup_ui(self) -> None:
        ...
        # --- Informativo ---
        self.info = QLabel(
            "Aplica erosão seguida de dilatação, removendo\n"
            "pequenas saliências e ruídos claros sem alterar\n"
    ...
    def processar(self, imagem: np.ndarray) -> np.ndarray:
        """Aplica a abertura morfológica na imagem.

        A abertura é definida como uma erosão seguida de dilatação
        (``cv2.MORPH_OPEN``).
        ...
        -------
        np.ndarray
            Imagem após a operação de abertura.
        """
        tamanho = self._obter_tamanho_kernel()
        ...
        if possui_alpha:
            ...
            resultado_rgb = cv2.morphologyEx(
                rgb, cv2.MORPH_OPEN, kernel, iterations=iteracoes,
            )
            resultado = np.dstack((resultado_rgb, alpha))
        else:
            resultado = cv2.morphologyEx(
                imagem, cv2.MORPH_OPEN, kernel, iterations=iteracoes,
            )
