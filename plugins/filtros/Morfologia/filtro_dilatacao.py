...
class FiltroDilatacao(PluginBase):
    """Plugin que aplica o filtro morfológico de Dilatação em imagens RGB.

    A dilatação expande as regiões claras (ou objetos brancos em imagens
    binárias), preenchendo pequenos buracos e conectando componentes
    próximos. O efeito é controlado pela forma e tamanho do elemento
    estruturante e pelo número de iterações.
    """

    display_name = "Dilatação"
    ...
    def processar(self, imagem: np.ndarray) -> np.ndarray:
        ...
        if possui_alpha:
            rgb = imagem[..., :3]
            alpha = imagem[..., 3]
            resultado_rgb = cv2.dilate(rgb, kernel, iterations=iteracoes)
            resultado = np.dstack((resultado_rgb, alpha))
        else:
            resultado = cv2.dilate(imagem, kernel, iterations=iteracoes)
