...
import cv2
import numpy as np
from PySide6.QtCore import Qt, QSettings, Signal, QSize, QTimer, qInstallMessageHandler, QDir, QStandardPaths
...
# Inicialização de configurações persistentes usando QSettings
config = Config()
...
def _instalar_filtro_mensagens_qt() -> None:
    ...
    _HANDLER_MENSAGENS_QT_ANTERIOR = qInstallMessageHandler(
        _HANDLER_MENSAGENS_QT)
        ...
        if (
            ...
            for classe_plugin in classes:
                ...
                acao.triggered.connect(
                    lambda _checked=False, cls=classe_plugin: janela_principal.abrir_plugin(
                        cls)
                )
...
class DocumentoImagem(QWidget):
    ...
    def atualizar_visualizacao(self, imagem_bgr: np.ndarray, ajustar_a_janela: bool = False) -> None:
        ...
        self.visualizador.definir_pixmap(
            pixmap, ajustar_a_janela=ajustar_a_janela)
        if ajustar_a_janela:
...
class TelaBoasVindas(QWidget):
    ...
    def __init__(self, janela_principal: "JanelaPrincipal", parent=None):
        ...
        area_arrastar.arquivo_solto.connect(
            self.janela._carregar_imagem_do_caminho)
        ...
        layout_placeholder.addSpacing(12)
        layout_placeholder.addWidget(
            area_arrastar, alignment=Qt.AlignmentFlag.AlignCenter)
        layout_placeholder.addStretch()
...
class BarraAbas(QTabBar):
    ...
    def paintEvent(self, event):
        ...
        direita_abas = max(self.tabRect(i).right()
...
class JanelaPrincipal(QMainWindow):
    ...
    def _construir_interface(self) -> None:
        ...
        self._toolbar_esquerda.ferramenta_alterada.connect(
            self._ao_ferramenta_alterada)
        self._toolbar_esquerda.modo_zoom_alterado.connect(
            self._ao_modo_zoom_toolbar_alterado)
        ...
        self._sidebar_direita.ajuste_solicitado.connect(
            self._ao_ajuste_solicitado)
    ...
    def abrir_imagem(self) -> None:
        diretorio = config.get_last_image_path()
        ...
        # Atualiza o último diretório usado para abrir imagens
        pasta = str(Path(caminhos[0]).parent)
        config.set_last_image_path(pasta)
    ...
    def _carregar_imagem_do_caminho(self, caminho: str) -> None:
        ...
        if imagem_bgr is None:
            QMessageBox.critical(
                self, "Erro", f"Não foi possível abrir:\n{caminho}")
            return
    ...
    def _solicitar_fechamento_aba(self, indice: int) -> None:
        ...
        checkbox_nao_mostrar = QCheckBox(
            "Não mostrar este aviso novamente", msg_box)
    ...
    def _adicionar_botao_fechar_aba(self, indice: int, widget: QWidget) -> None:
        ...
        botao.clicked.connect(
            lambda _checked=False, alvo=widget: self._fechar_aba_por_widget(
                alvo)
        )
    ...
    def salvar_imagem(self) -> None:
        ...
        if not isinstance(aba_atual, DocumentoImagem):
            QMessageBox.information(
                self, "Aviso", "Nenhuma imagem para salvar.")
            return
        ...
        if sucesso:
            ...
            self.statusBar().showMessage(
                f"Imagem salva em: {caminho_normalizado}")
    ...
    def colar_imagem_clipboard(self) -> None:
        ...
        arr_rgb = np.array(ptr).reshape(
            (altura, bytes_por_linha))[:, :largura * 3]
    ...
    def abrir_plugin(self, classe_plugin: type) -> None:
        ...
        dialogo.preview_requested.connect(
            lambda rgb: self._ao_receber_preview(rgb, aba_atual))
        dialogo.apply_requested.connect(
            lambda rgb: self._ao_aplicar_plugin(rgb, aba_atual))

        if hasattr(dialogo, "multiple_images_requested"):
            dialogo.multiple_images_requested.connect(
                lambda imagens: self._ao_aplicar_multiplas_imagens(
                    imagens, aba_atual)
            )

        # Se o usuário fechar sem aplicar, restaura a imagem original
        dialogo.finished.connect(
            lambda codigo: self._ao_fechar_plugin(codigo, aba_atual))
    ...
    def _ao_ferramenta_alterada(self, ferramenta: str) -> None:
        ...
        visualizador = getattr(aba_atual, "visualizador",
                               None) if aba_atual else None
    ...
    def _exibir_imagem(self, imagem_bgr: np.ndarray, ajustar_a_janela: bool = False) -> None:
        ...
        if aba_atual and hasattr(aba_atual, "atualizar_visualizacao"):
            aba_atual.atualizar_visualizacao(
                imagem_bgr, ajustar_a_janela=ajustar_a_janela)
